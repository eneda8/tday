...
const {isLoggedIn, isVerified, isAuthor, validatePost, correctCookies, setPostedToday, blockDuplicatePost, checkPostStreak, searchAndFilterPosts} = require("../middleware");
...
router.route("/")
    ...
    .post(isLoggedIn, isVerified, upload.single("image"), validatePost, correctCookies, setPostedToday,  blockDuplicatePost, checkPostStreak, catchAsync(posts.createPost));

router.route("/search")
    .get(isLoggedIn, isVerified, correctCookies, setPostedToday, catchAsync(searchAndFilterPosts), catchAsync(posts.search))

router.get("/new", isLoggedIn, isVerified, correctCookies, setPostedToday, blockDuplicatePost, checkPostStreak, catchAsync(posts.renderNewForm));

router.route("/:id")
    .get(isLoggedIn, isVerified, correctCookies, setPostedToday, checkPostStreak, catchAsync(posts.showPost))
